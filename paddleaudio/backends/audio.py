...
def resample(y,src_sr, target_sr):
    ...
    assert type(target_sr) == int and target_sr > 0 and target_sr <= 48000, 'make sure type(sr) == int and sr > 0 and sr <= 48000,'
...
def depth_convert(y,dtype): # convert audio array to target dtype
    ...
    if y.dtype not in __supported_dtype__:
        assert False, 'Unsupported audio dtype,  y.dtype is {}, supported dtypes are {}'.format(y.dtype,__supported_dtype__)
    if dtype not in __supported_dtype__:
        assert False, 'Unsupported dtype,  target dtype is {}, supported dtypes are {}'.format(dtype,__supported_dtype__)
